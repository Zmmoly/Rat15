...
class CommandHandler(private val context: Context) {
        ...
        return if (packageName != null) {
            try {
                ...
                if (intent != null) {
                    ...
                    "❌ التطبيق $appName غير مثبت"
                }
            } catch (e: Exception) {
                "❌ خطأ في فتح $appName"
            }
        } else {
            "لم أجد التطبيق \"$appName\". التطبيقات المتاحة:\n• واتساب\n• انستقرام\n• يوتيوب\n• الكاميرا\n• الإعدادات"
        }
    }
    ...
    private fun makeCall(contactName: String): String {
        ...
        if (contactName.matches(Regex("^[0-9+]+$"))) {
            try {
                ...
                context.startActivity(intent)
                return "✅ جاري الاتصال بـ $contactName"
            } catch (e: Exception) {
        ...
        return "⚠️ للاتصال بجهة اتصال، سأحتاج إذن الوصول لجهات الاتصال.\n\nيمكنك كتابة الرقم مباشرة، مثل:\naتصل 0501234567"
    }
    ...
    private fun closeApp(appName: String): String {
        ...
        return if (service != null) {
            ...
            "✅ جاري إغلاق $appName من Recent Apps"
        } else {
